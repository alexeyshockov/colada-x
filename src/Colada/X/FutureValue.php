 ...
 * Functor.
 *
 * Will be NOT implemented: __isset(). Useless, because calling isset() or empty() on future value...
 * Will be NOT implemented: __unset(). Useless, because calling unset() on future value...
 *
 * Will be NOT implemented: \Iterator support. Iterate over future value?.. Sounds strange.
 * Will be NOT implemented: \Countable support. count() call on future value sounds strange.
 *
 * @internal
 *
...
class FutureValue implements \ArrayAccess
{
     ...
     * @return callable
     */
    public function __toClosure()
    {
        $mapper = $this->mapper;
        return function($value) use($mapper) {
            // In the end return original value.
            return $mapper($value)->__getValue();
        };
    }
